...
// COMPETITION BUILD: Gemini-only API key input
// Production version supports multiple providers (Claude, OpenAI, Gemini)
...
export default function SettingsModal({ onClose, onSave, currentKey }) {
  ...
  const gemini = {
    label: 'Gemini',
    emoji: '🔵',
    desc: 'Google no permite llamadas directas desde el navegador, así que tu key se reenvía a través de nuestro servidor y no se guarda.',
    placeholder: 'AIza...',
    ...
    <div style={{ position:'fixed',inset:0,background:'rgba(6,13,31,0.85)',backdropFilter:'blur(8px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300,padding:'20px',animation:'fadeIn .2s ease' }}
      ...
      <div style={{ background:'var(--bg1)',border:'1px solid var(--blue-bd)',borderRadius:'var(--r-xl)',padding:'32px',width:'100%',maxWidth:'440px',animation:'fadeUp .3s ease' }}>
        ...
        <div style={{ padding:'16px',borderRadius:'var(--r-md)',border:`1px solid ${isFreeMode?'var(--blue-bd)':'var(--bd)'}`,background:isFreeMode?'var(--blue-dim)':'transparent',marginBottom:'10px' }}>
          ...
          <p style={{ fontSize:'12px',color:'var(--t3)',lineHeight:1.5 }}>3 reuniones por hora. Sin registro. El servidor aporta la API key (Gemini).</p>
        </div>
        ...
        <div style={{ padding:'16px',borderRadius:'var(--r-md)',border:`1px solid ${!isFreeMode?'var(--blue-bd)':'var(--bd)'}`,background:!isFreeMode?'var(--blue-dim)':'transparent',marginBottom:'24px' }}>
          <div style={{ display:'flex',alignItems:'center',gap:'8px',marginBottom:'12px' }}>
            <span>{gemini.emoji}</span>
            <span style={{ fontSize:'14px',fontWeight:600,color:!isFreeMode?'var(--blue)':'var(--t1)' }}>Tu API key {gemini.label} · ilimitado</span>
            {!isFreeMode&&<span style={{ fontSize:'10px',padding:'2px 7px',borderRadius:'4px',background:'var(--blue-dim)',color:'var(--blue)',fontWeight:700,border:'1px solid var(--blue-bd)' }}>ACTIVO</span>}
          </div>

          <p style={{ fontSize:'12px',color:'var(--t3)',lineHeight:1.5,marginBottom:'12px' }}>{gemini.desc}</p>

          <div style={{ position:'relative' }}>
            <input type={visible?'text':'password'} value={key} onChange={e=>setKey(e.target.value)} placeholder={gemini.placeholder}
          ...
          <p style={{ fontSize:'11px',color:'var(--t3)',marginTop:'6px' }}>
            Consigue tu key en{' '}
            <a href={gemini.keyUrl} target="_blank" rel="noreferrer" style={{ color:'var(--blue)',textDecoration:'none' }}>{gemini.keyUrlLabel}</a>
          </p>
        </div>
        ...
        <div style={{ display:'flex',gap:'8px' }}>
          ...
          {!isFreeMode&&<button onClick={()=>{onSave('');onClose()}} style={{ padding:'11px 14px',borderRadius:'var(--r-sm)',border:'1px solid var(--red-bd)',color:'var(--red)',fontSize:'13px' }}>Quitar</button>}
          <button onClick={()=>{onSave(key.trim());onClose()}} style={{ flex:2,padding:'11px',borderRadius:'var(--r-sm)',border:'none',background:'var(--blue)',color:'var(--bg0)',fontSize:'13px',fontWeight:700 }}>
            {hasKey?`${gemini.emoji} Usar mi key de ${gemini.label}`:'🌐 Modo gratuito'}
          </button>
        </div>
      </div>
    </div>
